...
is covered separately in test_info_disclosure_oracle.py.
"""
...
def _prober(status: int, body: str) -> InfoDisclosureProber:
    return InfoDisclosureProber(fire_probe=lambda: InfoDisclosureProbe(status=status, body=body))
...
def test_confirmed_disclosure_yields_a_confirmed_result() -> None:
    prober = _prober(500, f"stack trace: {_JASPER_MARKER}")
...
def test_plain_error_page_yields_no_confirmation() -> None:
    prober = _prober(500, "<html>Internal Server Error</html>")
...
def test_marker_on_a_200_still_confirms() -> None:
    ...
    prober = _prober(200, _JASPER_MARKER)
    result = detect_info_disclosure(prober)
    assert result.confirmed is True
