...
Hermetic (no network). Covers: flag gating, floor semantics (operator's
manual selection can only be strengthened, never weakened), disruptive-
category rejection, empty-graph, and no-op-when-nothing-new-to-add.
"""
...
def test_disruptive_category_is_rejected_not_passed_through(
...
def test_llm_can_add_a_category_the_operator_left_unset(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("REACHAGENT_RECON_DEPTH_TUNING", "1")
