...
shape: flag gating, floor semantics (operator's manual selection can only be
strengthened, never weakened), invalid-choice rejection, empty-graph, and
no-op-when-nothing-new-to-add.
"""
...
def test_valid_escalation_beyond_the_floor(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_no_escalation_when_llm_sees_nothing_worth_it(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("REACHAGENT_WORDLIST_DEPTH_TUNING", "1")
    tuner = FakeTuner({"size": "", "tech": ""})
    assert propose_wordlist_escalation(_graph_with_host().hosts(), client=tuner) is None


def test_invalid_size_and_tech_are_rejected_not_passed_through(
...
def test_operator_floor_is_never_dropped_even_if_llm_asks_for_less(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_llm_can_add_a_tech_hint_the_operator_left_unset(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("REACHAGENT_WORDLIST_DEPTH_TUNING", "1")
...
def test_llm_can_escalate_beyond_an_explicit_small_floor(monkeypatch: pytest.MonkeyPatch) -> None:
