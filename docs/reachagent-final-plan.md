...
**Status: Locked — v2.6 (Capability-Expansion Phase 2, W11).** Changes from v2.5: **(W11) Extended browser recon for SPA targets** — `browser/shim.py` gains a new, independent `run_browser_recon_async` function (never modifying the existing, well-tested XSS taint shim): a separate `RECON_SHIM_JS` init script hooks `console.*`/`fetch`/`XMLHttpRequest.open`, and `RECON_COLLECT_JS` (evaluated once the SPA has settled) reads localStorage/sessionStorage **key names only** (never values — the same "secrets never leave the browser process" discipline already applied to cookies), rendered `<form>`s, and links. New `scan/browser_recon.py` folds discovered forms into new `Endpoint`/`Parameter` graph facts by reusing `recon/surface.py`'s existing `_form_endpoint` builder (no parallel endpoint-spec mechanism) — so a form only a JS framework renders (invisible to the static HTML parser, which sees only the empty `<div id="root">` shell) becomes real, visible surface. Wired into `scan_all_classes` right after `firer`/`identity` are constructed and before Phase-3 dispatch, fail-open (a browser/Playwright crash degrades to a logged event, never aborts the scan). **Disclosed scope limit**: newly-materialized parameters are not automatically fingerprinted (`inferred_sink_type` stays unset) — sink-type inference is a separate, on-demand Explorer pass in the live agentic loop that this driver does not safely trigger inline; a follow-up would wire that. 21 new tests across 3 files.

**Status: Locked — v2.5 (Capability-Expansion Phase 2, W6).** Changes from v2.4: **(W6) Per-role model tiering** — `build_openai_compatible_client(tier=...)` gains an optional `tier: "core" | "grunt"` parameter (default `"core"`, fully backward compatible). `"grunt"` resolves an optional `REACHAGENT_LLM_GRUNT_MODEL` (same provider/key/base_url, just a cheaper model string) for high-volume, low-stakes tuning calls — wired into `rank_vuln_classes` (Phase-3 class-priority ordering) and the six recon tuning helpers (`recon/{transport,surface,signal,live,payload,vuln}_tuning.py`). Core reasoning call sites (the chat `/ask` persona, `DefaultLoopAdvisor.advise()`'s phase-boundary decisions, the report LLM narrative) are deliberately left untiered — no code change, by design, since those are exactly the "stronger model" targets. No grunt model configured → byte-for-byte identical behavior to before. New named-provider field `grunt_model` (optional, GUI settings panel + `config/providers.json`), threaded as `REACHAGENT_LLM_GRUNT_MODEL` through the existing `named_overrides`-application mechanism — no new plumbing. **Disclosed scope limit**: the recon per-step tool-selection loop (`_select_recon`, itself a genuinely high-volume "grunt" candidate per the plan's own framing) shares one client object with the upfront `plan_execution()` strategic call inside `llm/planner.py::build_planner_client()` — splitting that safely into two separately-tiered clients needs more surgery (client-lifecycle/error-handling for two objects) than fits this increment, so it is left untiered for now, honestly. Also fixed two pre-existing ruff line-length violations in `tests/gui/test_gui_slices.py`/`tests/scan/test_suspected_tier.py` that had slipped past an earlier commit's check (`src/`-only ruff runs missed test-file-only lines).

**Status: Locked — v2.4 (Capability-Expansion Phase 3.5).** Changes from v2.3: **(W17) Attack-path chaining** — a confirmed nosqli/ldap auth-bypass whose probe response carries REAL, reusable session material (a token or cookies — reusing `identity.login._extract_session_material`, the SAME parser the real login flow uses, never a heuristic guess) now spawns a fresh `AuthState.SYNTHETIC`/`Provenance.DERIVED` identity and re-hunts the full Phase-3 class order under it, exactly ONCE (never a chain-of-chains — the plan's explicit bounded-depth line). New `RequestFirer.register_identity()` — the one additive seam needed, since `_identity_stores`/`_identity_headers` were otherwise frozen at construction with no way for a mid-scan-derived identity to authenticate through the same firer. New `scan/chaining.py` (`capture_bypass_session`, `spawn_derived_identity`) reuses `IdentityStore.add`/`open_session` end to end — no parallel credential mechanism. New findings link back to the confirming finding via the existing `enables` edge (already rendered by `chain_paths()`/the report/GUI). Closes a real, previously-disclosed gap: `NoSqliResult`/`LdapiResult.bypass_identity_hint` was computed by the detectors but silently dropped by both orchestrator callers — confirmed via research before building, alongside the discovery that `ChainSolver.advance()`'s spawn branches were fully implemented but had zero live callers. **Disclosed scope limit**: re-hunts the SAME already-discovered endpoint set under the new identity's privilege — it does not trigger new content discovery, so an admin-only route never crawled unauthenticated stays invisible; and this increment is wired only into the sequential (non-concurrent-specialist) Phase-3 path, deliberately, to avoid touching Build Order 2c's already-adversarially-reviewed thread-safety guarantees.

**Status: Locked — v2.3 (Capability-Expansion Phase 3, partial).** Changes from v2.2: **(W9) Prototype pollution detection** — a new client-side STRUCTURAL check (`PROTOTYPE_POLLUTION`): a headless browser loads each HTML endpoint with an injected `__proto__`-shaped query parameter (3 common encodings), then evaluates whether a brand-new `{}` object literal carries the marker property — no baseline/differential needed, since a fresh POJO can never have an arbitrary property unless the page's own client-side merge logic actually polluted `Object.prototype`. New `scan/prototype_pollution.py`, same Playwright-driving shape as the existing `scan/xss_dom.py` (a testable driver-accepting core function + untested browser launch/close plumbing). New §5 coverage row (Partial — client-side only, by design). Also verified and documented (no code needed): JWT forgery (W7) and the two-identity IDOR/BOLA precondition (W8) were already fully built, more rigorously than the reference projects.

**Status: Locked — v2.2 (Capability-Expansion Phase 2, partial).** Changes from v2.1: **(W4) Aggressive mode** — an opt-in GUI toggle (`REACHAGENT_AGGRESSIVE`, default off) that bypasses the signal-gate precondition so signal-gated tools (nuclei/sqlmap/dalfox) fire even without a prior class signal in the graph; every emitted candidate still passes the oracle reconfirm (→ confirmed) or lands in the Suspected tier (→ W2), and scope/binary/memory/read-only gates are untouched. Read directly from the scan-scoped env (not `flag_enabled`, whose contextvar branch shadows env during a scan). **(W18) Application-domain inference** — one bounded LLM call (`scan/app_domain.py`) classifies WHAT the app is (hospital records / ecommerce / banking / …) from the observed surface; stored as a new order-only `Host.app_domain` fact, folded into the class-priority `signals` dict and surfaced as an `assistant-note` chat event, fail-open to no-profile. Remaining Phase 2+ items (interleaved hunting, per-role model tiering, detection depth, extended browser recon, attack-path chaining, LLM-authored report) still pending.

**Status: Locked — v2.1 (Capability-Expansion Phase 1).** Changes from v2.0/v1.20: the first phase of the v2 capability expansion (make ReachAgent behave like Shannon+Strix+CAI+PentAGI combined while keeping the proof gate none of them have). Three items shipped: **(1) Real conversational agent** — `OpenAICompatibleClient.chat(messages)` (multi-turn, real `system` role) plus `POST /api/scan/{id}/ask`, replacing the old fake one-way "Noted…" steer reply with a genuine LLM answer generated from live scan state; the chat box now both answers and steers, and the agent's own loop-decision rationale streams into the chat as `kind:"assistant-note"` events. The chat persona is read-only Q&A/steering — it can never call `write_finding`/`run_oracle` or bypass the oracle. **(2) Suspected / Unconfirmed tier** — a new structurally-separate `SuspectedFinding` graph node (mirrors `StaticAdvisory`; never a `Finding`, never `write_finding`) surfaces a genuine external assertion the oracle couldn't independently re-prove: a signal-gated scanner (nuclei/sqlmap/dalfox/jwt-tool) claim that failed reconfirmation, or that had no oracle mapping at all for its class. Deliberately does **not** fire on a routine oracle non-confirm inside ReachAgent's own systematic per-parameter drivers (nosqli/ldap/command_injection/jwt_forgery) — that outcome is the expected, common result for a properly-secured parameter, not a lead, and recording it would flood the tier with clean-negative noise (a design mistake caught and reverted in-session before shipping). Rendered in a permanently separate report section and a distinct GUI area, never counted in confirmed severity stats — so "no Finding without run_oracle" holds by construction. **(3) Cancel-feedback fix** — `cancelling` is now a distinct lifecycle (was collapsed to `running`), the Cancel button disables + relabels "Cancelling…" during the wind-down, header buttons have a stable min-width so the Pause↔Resume swap can't shift Cancel under the cursor, and a repeat cancel is an idempotent no-op (no duplicate event) — fixing the live accidental-double-cancel. The remaining v2 phases (aggressive mode, interleaved hunting, per-role model tiering, JWT-forgery/two-identity-IDOR/prototype-pollution/cache-poisoning depth, extended browser recon, attack-path chaining, app-domain inference, LLM-authored report) are tracked in the build plan and land phase by phase.

**Status: Locked — v1.20.** Changes from v1.19: **white-box (source-available) mode — Build Order 7, two new §5 rows.** Additive to, never a replacement for, the live black-box scan; off unless the operator supplies a local repo path. Four new fact-only graph node types (`SourceFile`, `PackageDependency`, `Secret`, `StaticAdvisory` — named `PackageDependency` rather than `Dependency` since `add_dependency` already exists as an unrelated edge writer), same "facts only, never a `Finding`" discipline as `Host`/`Service`. `StaticAdvisory` (a known-CVE match against a manifest dependency version) is the plan's one narrow, explicit exception to "no Finding without a confirmed `run_oracle` result" — a structurally separate node type that never touches `Finding`/`add_finding`/`write_finding` at all, so the non-negotiable holds by construction. New `whitebox/` package: `tools/base.py`'s `SourceToolRunner` is a deliberate parallel to (not a subclass of) the recon tier's `ReconToolRunner` — that base's scope gate coerces its target into a URL and checks its host, which a local filesystem path doesn't have — reusing only the generic RLIMIT_AS/GOMEMLIMIT subprocess discipline directly; `tools/semgrep.py` (SAST, result shape live-verified against a real semgrep run) and `tools/secrets.py` (TruffleHog, targeting the modern v3 filesystem-scan JSON-lines CLI, disclosed as hermetically-tested-only since this session's own sandbox has an older/incompatible legacy trufflehog installed); `sca.py` parses `requirements.txt`/`package.json` (pinned versions only) and reuses Build Order 4's already-built rate-limited/cached/EPSS-enriching NVD client for the CVE match — no new `trivy`/`pip-audit` dependency, live-verified end to end against the real NVD API. SAST hits and known-vulnerable dependencies fold into the same bounded `signals` dict `rank_vuln_classes` already reads (order-only, never gates). GUI gains an optional repo-path text field, validated as an existing local directory at the API boundary. Professional report gains a permanently separate "Static Analysis (White-Box — Unconfirmed Reachability)" section. No seventh oracle family, no `OracleMechanism` change.

**Status: Locked — v1.19.** Changes from v1.18: **known-vulnerable-component-version (new class, §5 row added) + §5 catch-up for four classes shipped without a matrix row.** New `cve_intel/` package (hexstrike-ai audit refinement, Attacker-Agent plan Build Order 4): `nvd_client.py` is a rate-limited (~5 req/30s, no API key), backoff-retrying, TTL-cached client for NVD's public CVE API plus FIRST.org's EPSS API — both budgets fail OPEN (skip enrichment) rather than blocking the scan when exhausted, the concrete failure mode the audit named in an unlimited hexstrike-ai equivalent. `detector.py` adds the STRUCTURAL oracle's new `KNOWN_VULNERABLE_VERSION` check type (`run_known_vulnerable_version`): reads the graph's own already-fingerprinted `Host.technology`/`detected_version`, asks NVD for known CVEs, and — only when a match exists — fires one live re-probe confirming the raw version substring is still genuinely present (deliberately no status-code gate, unlike a traversal payload: a version banner is just as real on a 403/404/500 error page as a 2xx). Severity derives from the worst matched CVSS score; EPSS scores ride along in finding metadata per finding, feeding the professional report template's CVSS+EPSS prioritization. Also backfills §5 rows that should have landed with their own classes: cloud storage bucket exposure, default credentials, login rate-limit absence, and information disclosure — all already implemented and tested, just missing their matrix entries until now. No seventh oracle family, no `OracleMechanism` change — everything above lives inside STRUCTURAL.

**Status: Locked — v1.18.** Changes from v1.17: **subdomain takeover (new class, §5 row added) + JWT `kid`-injection payload + a GUI/recon resilience pass.** `subdomain_takeover/detector.py` — a discovered subdomain's CNAME (now an attribute on `Host`, populated by `dnsx -cname`) matched against an embedded ~8-entry fingerprint table of known cloud-service "unclaimed" markers (S3, GitHub Pages, Heroku, Azure Web Apps, Read the Docs, Surge, Fastly, Ghost); one read-only GET to the CNAME target routes through the existing STRUCTURAL oracle's new `SUBDOMAIN_TAKEOVER` check type. That probe deliberately bypasses the scanned target's own `ScopeGuard` via a dedicated transport outside `RequestFirer` (`run_subdomain_takeover`) since the CNAME target is a third-party domain by construction — narrated into the live event feed for auditability instead. JWT forgery gains a fourth precomputed variant, `kid`-injection (`../../.../dev/null` path traversal + empty-key HMAC), alongside the existing none-alg/HS256-confusion/weak-secret shapes. Separately, a resilience pass found while investigating a real machine freeze: `katana`'s headless-browser flag (`-hl`) removed entirely (an orphan-prone internal Chromium spawn no profile ever enabled) and default `-mdp`/`-ct` caps added so an unbounded crawl can't grow its URL frontier without limit; the Playwright-backed XSS/browser tools (`run_xss_dom`, `fire_browser`, `fill_form`) now bound their probe in `asyncio.wait_for` since `page.evaluate()` has no built-in timeout and a hung script could pin a browser process forever; a critical `ScopeGuard.from_raw()` fix normalizes a full-URL "in-scope hosts" entry to its bare host (the GUI's own default value was a full URL, so every default-flow scan was silently refusing every request as out-of-scope and producing zero findings); a new `_MIN_FREE_MEMORY_MB` gate skips spawning another recon-tier/signal-gated process when the system is critically low on memory (advisory, fails open when unmeasurable); a durable rotating file log (`logging_setup.py`) now backs the `reachagent` logger namespace so a scan crash leaves a real traceback on disk; and the GUI gained a genuine Pause/Resume (reusing the existing cancel-checkpoint contract) plus a confirm-before-cancel guard. No seventh oracle family, no `OracleMechanism` change — subdomain takeover lives inside STRUCTURAL.

**Status: Locked — v1.17.** Changes from v1.16: **request smuggling, Weak → Partial** — new `smuggling/` package: `raw_probe.py` builds a byte-exact CL.TE-ambiguous HTTP/1.1 request over a plain socket (the one live-transport primitive a conforming HTTP client refuses to send — httpx normalizes away self-contradictory Content-Length/Transfer-Encoding pairs), `detector.py` sequences the measured latencies through the existing TIMING_STATISTICAL oracle completely unchanged (same ≥10-trial paired rigor as blind SQLi). Verified against a real local TCP server simulating both a confused (TE-reading) and a correct (CL-reading) backend — not just fabricated latency numbers — proving the actual bytes on the wire produce the claimed timing gap. CL.TE direction only; TE.CL and the full front-end/back-end topology this class needs to be exploitable in practice are documented gaps, not silent ones. No seventh oracle family. Also: `feroxbuster` now ranks ahead of `gobuster` in the content-discovery fallback order (materially higher throughput, native wildcard filtering); recon-tier and signal-gated tool wrappers now surface their real command line and a redacted stdout preview in the GUI's live event stream (previously invisible — only an outcome enum reached the operator).
...
| Auth & JWT issues (alg confusion, `none`, weak secret, `kid` injection) | **Full** | Structural — forged token grants access or it doesn't | Payload-backed since v1.11: none-alg / HS256-key-confusion / weak-secret precomputed tokens feed the existing `jwt_forgery` STRUCTURAL branch. `kid` path-traversal (`../.../dev/null`, empty-key HMAC) payload-backed since v1.18 — all four forged shapes fire in the same driver loop |
...
| Prototype Pollution (client-side) | **Partial** | Structural — an injected `__proto__`-shaped query parameter (3 encodings) is loaded in a headless browser, then a fresh `{}` object literal is evaluated in-page for the marker property; no baseline needed (a brand-new POJO can never have an arbitrary property unless the page's own merge logic polluted `Object.prototype`) | v2 W9. Client-side only — no server-side prototype-pollution check exists (would need a different, per-language-runtime signal); a page whose merge logic doesn't process query-string input at all is a true negative, not a miss |
...
| Request smuggling | **Partial** | TIMING_STATISTICAL — a raw-socket CL.TE-ambiguous request (Content-Length matches what's sent; a Transfer-Encoding reader's terminator leaves one stray byte) vs a well-formed baseline, same ≥10-trial paired-trial rigor as blind SQLi | CL.TE only (the TE.CL direction — front-end TE, back-end CL — isn't built); exactly one raw-socket transport a request library can't construct, added specifically for this (`smuggling/raw_probe.py`). Real front-end/back-end pair required for the class to exist at all — a single standalone server has nothing to desync against |
...
| Subdomain takeover | **Partial** | STRUCTURAL — a discovered subdomain's CNAME matches a known cloud-service pattern (S3, GitHub Pages, Heroku, Azure Web Apps, Read the Docs, Surge, Fastly, Ghost); one read-only GET to that CNAME target confirms the service's own "unclaimed" body marker | Detects an embedded fingerprint table's ~8 known services only — a service outside that table, or one that changed its unclaimed-state marker text, is undetected. The probe deliberately bypasses the scanned target's own `ScopeGuard` (the CNAME target is a third-party domain by construction) via a dedicated read-only-only transport outside `RequestFirer` — see `run_subdomain_takeover` |
| Cloud storage bucket exposure | **Partial** | STRUCTURAL — a generated S3/GCS/Azure bucket name (derived from the target's own hostname labels) both exists and is publicly listable, confirmed by the provider's own listing-response marker | Small, bounded naming-convention guess (~10 suffixes × up to 2 seed labels × 3 providers) — a bucket named outside that convention, or hosted on a provider outside S3/GCS/Azure, is undetected. Same disclosed-limit shape as subdomain takeover |
| Default credentials | **Full** | STRUCTURAL — a well-known username/password pair authenticates successfully against a discovered login form (real session material captured, not just a 2xx status) | Fixed 8-pair allowlist tried against the first discovered `html_form`-kind login only; a non-HTML-form login (GraphQL/JSON) is not applicable, not a miss |
| Login rate-limit absence | **Full** | STRUCTURAL — a small, bounded burst of wrong-credential attempts (never a real brute force) never triggers a 429 or a known lockout/throttle marker | Same "was a specific defensive mechanism present or absent" shape as clickjacking/CORS; a partial burst (network failure, a safety-gate refusal) carries no trustworthy signal and is reported inconclusive rather than guessed |
| Information disclosure (verbose errors/stack traces) | **Partial** | STRUCTURAL — a response body contains a known framework/language stack-trace or debug-page marker | Fixed ~8-entry marker table (Java/JSP, Python, Django, Spring, PHP, ...); a framework outside that table, or one whose debug page format differs, is undetected. Findings are labeled `severity="informational"` — a leaked implementation detail, not itself a proven exploit path |
| Known-vulnerable component version | **Partial** | STRUCTURAL — a fingerprinted `Host.technology`/`detected_version` (whatweb/recon fact) has a known CVE per a live NVD lookup (rate-limited/cached/EPSS-enriched, `cve_intel/nvd_client.py`); one live re-probe confirms the raw version substring is still genuinely present, not stale graph data | Depends entirely on whatweb's own fingerprint accuracy and NVD keyword-search relevance — a version whatweb doesn't recognize, or a product/version pair NVD's keyword search doesn't match well, goes undetected. NVD's public (no-API-key) rate limit (~5 req/30s) also bounds how many distinct product/version pairs get checked per scan; a lookup skipped for budget reasons fails open (not applicable), never blocks the scan |
| Multi-hop cross-class chains | **Full — core differentiator** | Chain Solver over `enables` / `derived_credential` edges | See §8 |
| Known-vulnerable dependencies (white-box, static) | **Weak, by design** | None — deliberately never oracle-confirmed. `StaticAdvisory` matches a manifest-pinned `name==version` against a live NVD lookup (reuses the Build Order 4 client) | The plan's one narrow, explicit exception to "no Finding without run_oracle": evidence is a manifest line + a public advisory reference, never a behavioral confirmation, because the vulnerable path may not be reachable from the live surface at all. Structurally a different node type from `Finding` (never `add_finding`/`write_finding`), rendered in its own permanently-separate report section. Requires an operator-supplied local repo path (off by default); only exact-pinned `requirements.txt`/`package.json` entries are read (a semver range or an un-resolved lockfile entry is skipped, not guessed); NVD's own keyword-search relevance is the same disclosed limitation `known_vulnerable_version` already carries |
| Static code findings (white-box SAST/secrets) | **Weak, by design** | None — a `SourceFile`/`Secret` hit is a fact-only node, same discipline as `Host`/`Service`, never a `Finding` | A pattern match or a detected credential location, never a proven exploit — deliberately steers Phase 3's live-testing priority (folded into the same bounded, order-only signals `rank_vuln_classes` already reads) and never gates or substitutes for oracle confirmation of anything. semgrep's `--config=auto` ruleset breadth and TruffleHog's own detector catalog are the only coverage ceiling; a class either tool doesn't have a rule for is undetected |
