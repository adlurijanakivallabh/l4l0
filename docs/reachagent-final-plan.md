...
**Status: Locked — v1.9.** Changes from v1.8: Phase 3 Juice Shop evaluation now records an evidence-backed API-only ceiling instead of an unattainable 75% claim. The pinned dockerized `/file-upload` route has no persistence/retrieval/execution signal, and tracker transitions are evaluation ground truth rather than generic findings. API-only scoring credits only typed, oracle-backed challenge claims; browser-capable runs remain separate. No new oracle family, no §5 rating change.
...
| **OWASP Juice Shop** | Target-specific API-only evaluation against the pinned image. The strict denominator is the nine keys in `VERIFIED_CHALLENGE_SCOPE`, not all tracker rows. Current deterministic `fire_request` → `run_oracle` → `write_finding` paths cover **4/9 (44.4%)**: the three SQLi auth-bypass keys and `nullByteChallenge`, whose poison-null package-artifact result is now labeled `file_upload` as an input-validation finding. `unionSqlInjectionChallenge` and `dbSchemaChallenge` are not credited because generic search divergence does not prove UNION extraction or schema disclosure. `uploadSizeChallenge` and `uploadTypeChallenge` lack persistence/retrieval/execution evidence: `/file-upload` returns empty `204` responses, rejects oversize input with Multer `500`, and has no artifact URL; timing overlap and an invalid `ok.jpg` baseline do not establish a bypass. `localXssChallenge` requires browser-rendered DOM execution and tracker attribution unavailable to the API-only runner. Generic §5 class ratings do not imply target-specific coverage. |
...
| **3 — Full class coverage** | 7–9 | OOB and paired-trial statistical oracle families (blind SQLi OOB-first + negative-control timing; NoSQLi and LDAP extraction reuse same statistical harness); Playwright taint-tracking shim for DOM XSS; file upload/path traversal. Target: Juice Shop + PortSwigger Academy blind-SQLi labs. | Juice Shop uses a clean pinned target and strict nine-key tracker scope. The planned **≥75% (≥7/9)** floor is not currently achievable in API-only Docker mode: the honest deterministic ceiling is **4/9 (44.4%)**—three SQLi auth-bypass keys plus the null-byte input-validation key, labeled `file_upload` for the tracker scope. Search UNION/schema claims, status-only upload transitions, and API readback XSS are not credited. False-positive rate must remain ≤10%; the gate becomes eligible only after browser-capable DOM attribution and persistent upload retrieval/execution evidence exist, or the target scope is explicitly re-baselined. Retain the PortSwigger blind-SQLi criterion unchanged. |
