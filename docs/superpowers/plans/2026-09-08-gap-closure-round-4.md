...
- [ ] Grep every file touched by this plan for any accidentally-leaked reference-project name (this project's own standing discipline — a prior sweep this same day found and fixed ~20 pre-existing leaks elsewhere in the codebase, so treat this as a real check, not a formality): run the project's standard reference-name leak grep (see CLAUDE.md's clean-room policy) over `git diff --name-only <base-commit>..HEAD` — expect "clean" (the pre-existing `_shannon_entropy` function in `core/redaction.py` is the standard information-theory term, unrelated to any reference-project name, and is not touched by this plan, so it will not appear in this plan's own diff).
